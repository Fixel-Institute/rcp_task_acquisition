...
class ControlsPanel(wx.Panel):
    ...
    def _set_up_tasks(self, task, button_width):
        ...
        self.task_sizer.Add(
            ...
            pos=(vertical_position, 0),
            span=(0, 5),
            flag=wx.RESERVE_SPACE_EVEN_IF_HIDDEN | wx.ALIGN_LEFT | wx.ALL,
            border=0,
        )
        vertical_position += 1
        ...
        self.hardware_button = wx.ToggleButton(
            self, id=wx.ID_ANY, label="Hardware Test", size=(button_width * 2, -1)
        )
        self.task_sizer.Add(
            self.hardware_button,
            pos=(vertical_position, 2),
            span=(0, 2),
            flag=wx.LEFT,
            border=white_space,
        )
        ...
        self.tens_button = wx.Button(
            self, id=wx.ID_ANY, label="TENS Test", size=(button_width * 2, -1)
        )
        self.task_sizer.Add(
            self.tens_button,
            pos=(vertical_position, 0),
        ...
        self.session_button = wx.ToggleButton(
            self, id=wx.ID_ANY, label="Start Task", size=(button_width * 4 + 15, -1)
        )
        self.task_sizer.Add(
            self.session_button,
            pos=(vertical_position, 0),
        ...
        self.quit = wx.Button(
            self, id=wx.ID_ANY, label="Exit to Launch Menu", size=(int(button_width * 4 + 15), -1)
        )
        self.task_sizer.Add(
            self.quit, pos=(vertical_position, 0), span=(0, 4), flag=wx.LEFT, border=white_space
        )
    ...
    def get_cam_handles(self):
        return (
            self.cam_panel.init,
            self.cam_panel.reset,
            self.cam_panel.update_settings,
            self.cam_panel.play,
            self.cam_panel.rec,
            self.cam_panel.exposure_button,
            self.cam_panel.set_crop,
            self.cam_panel.crop,
            self.cam_panel.minRec,
            self.cam_panel.secRec,
        )
...
class CameraControlPanel(wx.Panel):
    def __init__(self, parent, button_width):
        ...
        self.update_settings = wx.Button(
            self, id=wx.ID_ANY, label="Update Settings", size=(button_width * 2, -1)
        )
        camsizer.Add(
            self.update_settings, pos=(vpos, 6), span=(1, 6), flag=wx.ALL, border=white_space
        )
