...
class N_back(bases.StimulusBase):
    def __init__(self, base_vars, button):
        ...
        self.letters = {
            "D": visual.TextStim(self.display, text="D", name="trial", pos=(0, 0), height=800),
                ...
                if not self.is_real:
                    ...
                    if self.button_press:
                        if self.answer_list[index][t_index] == Answer.DIFFERENT:
                            ...
                            self.answer_list[index][t_index] == Answer.SAME
                            or self.answer_list[index][t_index] == Answer.NOGO
                        ):
                            timeData, fixFlipTime = self.showAndLog(
                                self.display, timeData, 0, correct_ims, 2, "Initial_Fixation_Shown"
                            )
                    elif (
                        self.answer_list[index][t_index] == Answer.DIFFERENT
                        or self.answer_list[index][t_index] == Answer.NOGO
                    ):
                        timeData, fixFlipTime = self.showAndLog(
                            self.display, timeData, 0, correct_ims, 2, "Initial_Fixation_Shown"
